...
class ScreenshotLlamaImageProcessor(BaseImageProcessor):
    ...
    def extract_flattened_patches(self, image: np.ndarray, patch_size: dict, concat_coord = True, **kwargs) -> np.ndarray:
        ...
        # result: the actual patch pixel values
        # image_token_ids: the special tokens corresponding to the image, including the patch token, the newline token, and the image start/end tokens
        result = to_numpy_array(result)
        image_token_ids = np.zeros((rows, columns + 1), dtype=np.int32) + PATCH_ID
        image_token_ids[:, -1] = NEWLINE_ID
        image_token_ids = [IMG_BEGIN_ID] + image_token_ids.flatten().tolist() + [IMG_END_ID]

        return result, image_token_ids
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        images_and_token_ids = [
            self.extract_flattened_patches(image=image, patch_size=patch_size, concat_coord=concat_coord)
            for image in images
        ]
        images = [image_and_token_ids[0].astype(np.float32) for image_and_token_ids in images_and_token_ids]
        image_token_ids = [image_and_token_ids[1] for image_and_token_ids in images_and_token_ids]

        # create attention mask in numpy
        attention_masks = [(image.sum(axis=-1) != 0).astype(np.float32) for image in images]

        encoded_outputs = BatchFeature(
            data={"flattened_patches": images, "attention_mask": attention_masks, "image_raw_token_ids": image_token_ids}, tensor_type=return_tensors
        )
...
class ScreenshotLlamaProcessor:
    ...
    def __init__(self, config_path, **image_processor_kwargs):
        ...
        self.tokenizer.add_tokens("</img>", special_tokens=True)
        self.tokenizer.add_tokens("<patch>", special_tokens=True)

        self.img_begin_token_id = self.tokenizer.encode("<img>")[-1]
        self.img_end_token_id = self.tokenizer.encode("</img>")[-1]
        self.patch_token_id = self.tokenizer.encode("<patch>")[-1]
        ...
        self.image_processor = ScreenshotLlamaImageProcessor(**self.image_processor_config)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image_input_ids = encoding_image_processor.pop("image_raw_token_ids")
        image_input_ids[image_input_ids == IMG_BEGIN_ID] = self.img_begin_token_id
        image_input_ids[image_input_ids == IMG_END_ID] = self.img_end_token_id
        image_input_ids[image_input_ids == PATCH_ID] = self.patch_token_id
        image_input_ids[image_input_ids == NEWLINE_ID] = self.newline_token_id

        encoding_image_processor["image_input_ids"] = image_input_ids
