...
from .span_masking import SpanMaskingGenerator
import sys
sys.path.append("..")
...
class PIXELEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.embedding_layernorm:
            self.layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
            print("*** Using embedding layernorm ***")
        self.num_patches = self.patch_embeddings.num_patches
        if int(self.num_patches ** 0.5) ** 2 < self.num_patches:
            self.num_patches = (int(self.num_patches ** 0.5)+1) ** 2
        else:
            self.num_patches = (int(self.num_patches ** 0.5)) ** 2
        ...
        self.config = config
        self.span_masking = False
        self.ignore_white_patches = False
        self.initialize_weights()

    def initialize_weights(self):
        # initialize (and freeze) position embeddings by sin-cos embedding
        pos_embed = get_2d_sincos_pos_embed(
            self.position_embeddings.shape[-1], int(self.num_patches ** 0.5), add_cls_token=True
        )
    ...
    def random_masking(self, sequence, attention_mask):
        ...
        if self.span_masking:
            ...
            if self.ignore_white_patches:
                patch_mask = np.array([span_masking_generator(num_text_patches=attention_mask[_].long().sum()) for _ in range(batch_size)])
            else:
                patch_mask = np.array([span_masking_generator() for _ in range(batch_size)])
            patch_mask = torch.tensor(patch_mask, device=sequence.device)
        ...
        if getattr(self, "avoid_masking_white", False): 
            noise[attention_mask.bool()] += 100.0
        ...
        return sequence_masked, attention_mask_masked, mask, ids_shuffle, ids_restore
    ...
    def forward(self, pixel_values, attention_mask=None, patch_mask=None):
        ...
        if getattr(self.config, "input_cnn", False):
            # pixel values (bsz, 3, h * ph, w * pw)
            pixel_values = self.cnn_activation(self.cnn(pixel_values)) # (bsz, 3, h * ph, w * pw)
        
        embeddings = self.patch_embeddings(pixel_values)

        # add position embeddings w/o cls token
        embeddings = embeddings + self.position_embeddings[:, 1:embeddings.size(1)+1, :]

        if getattr(self.config, "embedding_layernorm", False):
            embeddings = self.layernorm(embeddings)

        # masking: length -> length * config.mask_ratio
        if patch_mask is not None:
            embeddings, attention_mask, mask, ids_shuffle, ids_restore = self.controlled_masking(
                embeddings, attention_mask, patch_mask
            )
        else:
            embeddings, attention_mask, mask, ids_shuffle, ids_restore = self.random_masking(embeddings, attention_mask)
        ...
        return embeddings, attention_mask, mask, ids_shuffle, ids_restore
...
class PIXELModel(PIXELPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output, attention_mask, mask, ids_shuffle, ids_restore = self.embeddings(pixel_values, attention_mask, patch_mask)
...
class PIXELDecoder(nn.Module):
    def __init__(self, config, num_patches, dtype):
        ...
        phw = config.patch_size ** 2 if isinstance(config.patch_size, int) else config.patch_size[0] * config.patch_size[1]
        self.decoder_norm = nn.LayerNorm(config.decoder_hidden_size)
        self.decoder_pred = nn.Linear(
            config.decoder_hidden_size, phw * config.num_channels, bias=True
        )  # encoder to decoder
        self.gradient_checkpointing = False
        self.config = config
        self.dtype = dtype
...
class PIXELForPreTraining(PIXELPreTrainedModel):
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        if not self.config.check_dataset:
    ...
    def forward(
        ...
        # if getattr(self, "ignore_white_patches", False):
        #     fpatches = flattened_patches[:, :, 2:]
        #     non_white_patches = ((fpatches - fpatches.mean(dim=-1, keepdim=True)) ** 2 < 1e-6).long().sum(-1) != fpatches.shape[-1] # (bsz, len)
        #     reverse_non_white_patches = non_white_patches.long().flip(-1) # (bsz, len)
        #     white_patches = reverse_non_white_patches.argmax(-1)
        #     white_patches[reverse_non_white_patches.sum(-1) == 0] = reverse_non_white_patches.shape[-1]
        #     non_white_patches = reverse_non_white_patches.shape[-1] - white_patches

        #     rows, cols = torch.meshgrid(torch.arange(attention_mask.shape[0]), torch.arange(attention_mask.shape[1]))
        #     cols = cols.to(attention_mask.device)
        #     white_patch_mask = cols >= non_white_patches.unsqueeze(1)
        #     attention_mask[white_patch_mask] = 0
        if isinstance(self.config.patch_size, collections.abc.Iterable):
            ph, pw = self.config.patch_size
        else:
            pw = ph = self.config.patch_size 
        pixel_values = flattened_patches_to_vit_pixel_values(flattened_patches[:, :, 2:], height=self.config.image_size[0], width=self.config.image_size[1], patch_height=ph, patch_width=pw, image_mode=self.config.image_mode)
        outputs = self.vit(
            pixel_values,
...
class PIXELModelOutput(ModelOutput):
    ...
    mask: torch.LongTensor = None
    ids_shuffle: torch.LongTensor = None
    ids_restore: torch.LongTensor = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
...
def inject_flash_attention_pixel(model):
    for key, _ in model.named_modules():
        attention_name = "attention.attention"

        if key[-len(attention_name):] == attention_name:
            _, _, attn = find_module(model, key)
            print("Inject vision encoder/decoder flash attn:", key)
            attn.original_forward = attn.forward
            attn.forward = pixel_forward_flash_attn.__get__(attn, type(attn))
